...
(3.10 и 3.11), поэтому в релизе лежат оба набора, а запускалка выбирает свой
по `sys.version_info`.
...
Подробности и раскладка клавиш — `stand/README.md`. Стенд не заменяет проверку
на приборе: настоящие коды кнопок, фреймбуфер, скорость A53, звук и установку
обновления он не проверяет.
...
### Сборка релиза
    ...
    python3 tools/build_release.py --app app --out dist

Собирать нужно тем же Python, что стоит на целевой приставке — чужой `.pyc` не
читается вовсе. Наборы копятся в `dist/bundles` и уезжают в один архив;
порядок выпуска: сначала релиз с архивом, потом `update.json`, иначе приставки
увидят версию, которой ещё нет.

Значок для списка muOS рисуется отдельно:

    python tools/make_glyph.py
