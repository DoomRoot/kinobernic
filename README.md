...
Онлайн-кинотеатр и эфирное ТВ для портативных приставок Anbernic. Каталог,
поиск, выбор озвучки, продолжение просмотра и 741 телеканал — всё на самой
приставке, без телефона рядом и без торрентов.
...
Эфирные каналы — 741 строка: федеральные, региональные, кино, спорт, детские.
Список собирается на самой приставке за несколько секунд и лежит на карте.

У канала обычно не один поток, а несколько: «НТВ» лежит в списках тремя
строками, «ТНТ» — четырьмя. Показываем один канал, а запасные держим при нём и
пробуем сами, если узел молчит. Сколько их — видно прямо в строке.

Start отбирает каналы по названию, прямо по мере набора букв — до «Пятницы»
иначе тридцать нажатий крестовиной. Y ставит звезду, избранное уходит наверх
списка.

| Список | Отбор по названию |
|---|---|
| ![Телевизор](docs/screens/tv.png) | ![Отбор каналов](docs/screens/tv-search.png) |

Эфир ведёт себя как эфир: перемотки нет, серий нет, подсказки только «A» и «B».

![Канал идёт](docs/screens/tv-live.png)

*Этот снимок с RG35XX H (640×480): на muOS кадр из плеера снять нечем — экран
там держит система.*
...
| X | пересобрать каталог (на «Подборках» и «Телевизоре») | — |
| Y | звезда каналу (на «Телевизоре») | сезоны, серии и озвучки |
| L1 R1 | вкладки | скрыть подсказки |
| START | поиск, на «Телевизоре» — отбор каналов | — |
